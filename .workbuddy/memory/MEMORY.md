...
- **Husky hook 必带 shebang + LF**：`.husky/pre-commit` 第一行必须 `#!/usr/bin/env sh`（Git 在 Windows 上对无 shebang 的 hook 会报 `cannot spawn .husky/pre-commit: No such file or directory`，文件实际存在但被当作可执行文件直接 spawn，bash 拒绝）；同时 `.gitattributes` 需加 `.husky/** text eol=lf` 强制 LF，避免被 `core.autocrlf` 转换成 CRLF 后 bash 把 CR 当成命令一部分执行失败。重新加 +x 用 `git update-index --chmod=+x .husky/pre-commit`（Windows Git Bash 无 `chmod` 命令）。
