      ...
      <a-col :xs="24" :lg="16">
        ...
        <a-card title="基础资料" :bordered="false" class="mb-16">
          <a-descriptions :column="{ xs: 1, sm: 2 }" bordered size="small">
            ...
            <a-descriptions-item label="数量">{{ record?.数量 ?? '--' }}</a-descriptions-item>
...
const submitReview = async () => {
  ...
  const {
    ...
    分析单号, 来源编号, 来源, 工单单号, 排产用户, 电压,数量,
  } = props.record!;


  // 构造提交数据：基于原始记录，更新线圈、物料编号, 状态、最终交期、评审备注
  const reviewData = new PMCDeliveryReview({
    编号, 用户编号, 合同号, 排产编号,数量, 货号, 中文品名, 中文规格, 分析单号, 来源编号, 来源, 工单单号, 排产用户, 电压,
    ...
    特殊要求: reviewForm.specialRequirement, // 特殊要求
  });
