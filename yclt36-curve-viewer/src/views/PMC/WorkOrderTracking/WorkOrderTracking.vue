        ...
        >
          <template #bodyCell="{ column, record, text, index }">
            ...
            <template v-else-if="column.key === '齐套'">
              <span class="status-tag" :style="getKittingStyle(text)">
                {{ text }}
              </span>
            </template>
            <template v-else-if="column.key === '配料'">
              <span class="status-tag" :style="getFeedingStyle(text)">
                {{ text }}
              </span>
            </template>
            ...
            <template v-else-if="isLinkColumn(column.key)">
              <a-tooltip
                :title="text"
                placement="topLeft"
                :overlayStyle="{ maxWidth: '400px', wordBreak: 'break-all' }"
              >
...
import { kittingStatusOptions, feedingStatusOptions } from './data';
...
/** 工单总需求明细行（与 WorkOrderDetailModal 的 WorkOrderItem 结构一致） */
...
/** 物料需求明细行（与 WorkOrderDetailModal 的 MaterialItem 结构一致） */
...
const workOrderDetailList = ref<WorkOrderSalesControlDetail[]>([]);
...
const currentWorkOrderData = ref<WorkOrderDetailItem[]>([]);
const currentMaterialData = ref<MaterialDetailItem[]>([]);
...
function getEarliestDeliveryDate(item: WorkOrderSalesControl): string | null {
  ...
  const itemDetails = workOrderDetailList.value.filter((d) => String(d.货号 || '') === String(partNo));
...
async function runKittingAnalysis() {
  ...
  try {
    for (const item of tableData.value) {
      ...
      const allFeedingReady = materialRows.every((row) => Number(row.缺料数) === 0);
      // 齐套：所有子件 仓库数-需求数 都大于 0 才算齐套，否则缺料
      const allKittingReady = materialRows.every((row) => Number(row.仓库数) - Number(row.需求数) > 0);
...
const allTableRows = computed<TableRowData[]>(() => {
  const rows = filteredData.value.map((item) => {
    ...
    for (const date of deliveryDatesWithData.value) {
      ...
      deliveryMap[dateKey] = planInfo ? { quantity: planInfo.quantity, status: planInfo.status } : null;
    }
...
function generateWorkOrderDetail(record: TableRowData) {
  const matched = workOrderDetailList.value.filter((d) => String(d.货号 || '') === String(record.货号 || ''));
...
function generateWorkOrderDetailForDate(record: TableRowData, targetDate: string) {
  const matched = workOrderDetailList.value.filter(
    (d) => String(d.货号 || '') === String(record.货号 || '') && (d.交货日期 || '').substring(0, 10) === targetDate
  );
  if (matched.length === 0) return [];
  return matched.map((d, idx) => ({
    id: idx + 1,
...
async function generateMaterialDetail(record: TableRowData) {
  try {
    ...
    const bomList = await externalProductionService.getExternalProductionBOMList(
      new PMCRequestDto({ 货号: record.货号 })
    );
...
/** 从 BOM 列表提取去重后的货号列表 */
function extractGoodsNos(bomList: ExternalProductionBOM[]): string[] {
  const nos = bomList.map((d) => d.货号).filter((g): g is string => !!g);
  return Array.from(new Set(nos));
}

/** 按货号列表批量查询外产领料 */
async function queryPickMaterialByGoodsNos(goodsNos: string[]): Promise<ExternalProductionPickMaterial[][]> {
  return Promise.all(
    goodsNos.map((g) => externalProductionService.getExternalProductionPickMaterialList(new PMCRequestDto({ 货号: g })))
  );
}

/** 根据子记录查询父记录，构建「父级编号 → 父节点货号」映射 */
async function buildParentPartNoMap(childResults: ExternalProductionPickMaterial[][]): Promise<Map<string, string>> {
  const parentIds = Array.from(new Set(childResults.flatMap((list) => list.map((p) => p.父级编号).filter(Boolean))));
  const map = new Map<string, string>();
  if (parentIds.length === 0) return map;

  const parentResults = await Promise.all(
    parentIds.map((id) =>
      externalProductionService.getExternalProductionPickMaterialList(new PMCRequestDto({ 编号: id }))
    )
  );
...
async function buildOutQtyMap(
  childResults: ExternalProductionPickMaterial[][],
  targetPartNo: string
...
function mapApiItemToTableItem(item: WorkOrderSalesControl): WorkOrderSalesControl {
...
async function fetchData() {
  ...
  try {
    ...
    dataSource.value = (mainData || []).map((item) => mapApiItemToTableItem(item));
