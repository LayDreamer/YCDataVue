  ...
  <div class="ep-test-container">
    ...
    <a-row :gutter="[16, 16]">
      ...
      <a-col :xs="24" :md="12">
        <a-card title="外产领料" :bordered="false">
          <template #extra>
            <a-space>
              <a-button type="primary" @click="openAdd('pick')"> <PlusOutlined /> 新增 </a-button>
              <a-button danger @click="openDelete('pick')"> <DeleteOutlined /> 删除 </a-button>
      ...
      <a-col :xs="24" :md="12">
        <a-card title="外产入库" :bordered="false">
          <template #extra>
            <a-space>
              <a-button type="primary" @click="openAdd('warehousing')"> <PlusOutlined /> 新增 </a-button>
              <a-button danger @click="openDelete('warehousing')"> <DeleteOutlined /> 删除 </a-button>
        ...
        <a-card title="工单销控表" :bordered="false">
          <template #extra>
            <a-space>
              ...
              <a-button danger @click="openDelete('workOrder')"> <DeleteOutlined /> 删除 </a-button>
        ...
        <a-card title="工单销控表明细" :bordered="false">
          <template #extra>
            <a-space>
              ...
              <a-button danger @click="openDelete('workOrderDetail')"> <DeleteOutlined /> 删除 </a-button>
    ...
    <a-modal
      ...
      :title="
        deleteType === 'pick'
          ? '删除外产领料'
          : deleteType === 'warehousing'
            ? '删除外产入库'
            : deleteType === 'workOrder'
              ? '删除工单销控表'
              : '删除工单销控表明细'
      "
      @ok="handleDelete"
    ...
    >
      <a-form :label-col="{ span: 6 }" :wrapper-col="{ span: 16 }">
        <a-form-item
          :label="deleteType === 'pick' ? '领料编号' : deleteType === 'warehousing' ? '入库编号' : '记录编号'"
          required
        >
...
async function clearAllBom() {
  ...
  try {
    const list = await externalProductionService.getExternalProductionBOMList();
    const ids = (list || []).map((i) => i.编号).filter((id): id is string => Boolean(id));
    if (ids.length === 0) {
...
const workOrderList = ref<WorkOrderSalesControl[]>([]);
...
async function handleAdd() {
  ...
  try {
    if (addType.value === 'pick') {
      const existing = (
        await externalProductionService.getExternalProductionPickMaterialList(new PMCRequestDto({ 编号: addForm.编号 }))
      )[0];
      ...
      const existing = (
        await externalProductionService.getExternalProductionWarehousingList(new PMCRequestDto({ 编号: addForm.编号 }))
      )[0];
...
async function handleDelete() {
  ...
  try {
    ...
    if (deleteType.value === 'pick') {
      ...
      workOrderList.value = workOrderList.value.filter((i) => i.编号 !== deleteId.value);
      ...
      workOrderDetailList.value = workOrderDetailList.value.filter((i) => i.编号 !== deleteId.value);
