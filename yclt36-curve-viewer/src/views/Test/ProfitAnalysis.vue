  ...
  <div class="profit-analysis-container">
    <div class="page-header">
      ...
      <div class="header-right">
        ...
        <a-button type="primary" @click="exportReport"> <template #icon>📊</template> 导出报告 </a-button>
      </div>
    </div>
    ...
    <a-card class="progress-card" style="margin-bottom: 16px">
      <div class="payback-progress">
        ...
        <div class="progress-info">
          <span
            >已分红: <strong>{{ formatMoney(totalDividend) }}</strong></span
          >
          <span
            >剩余: <strong>{{ formatMoney(Math.max(0, totalInvestment - totalDividend)) }}</strong></span
          >
          <span
            >预计完成: <strong>{{ estimatedPaybackDate }}</strong></span
          >
        </div>
      </div>
    </a-card>
    ...
    <a-row :gutter="[16, 16]">
      ...
      <a-col :xs="24" :lg="8">
        <a-card title="� 两期对比分析" class="chart-card compare-phase-card" style="margin-top: 16px">
            ...
            <a-col :span="12">
              <div class="phase-box phase1">
                ...
                <div class="phase-stat">
                  ...
                  ><strong class="green">{{ formatMoney(phase1TotalDividend) }}</strong>
                </div>
                <div class="phase-stat">
                  <span class="label">月均分红</span><strong>{{ formatMoney(phase1AvgDividend) }}</strong>
                </div>
              </div>
            </a-col>
            <a-col :span="12">
              <div class="phase-box phase2">
                ...
                <div class="phase-stat">
                  <span class="label">累计分红</span
                  ><strong class="green">{{ formatMoney(phase2TotalDividend) }}</strong>
                </div>
                <div class="phase-stat">
                  <span class="label">月均分红</span><strong>{{ formatMoney(phase2AvgDividend) }}</strong>
                </div>
              </div>
            </a-col>
          </a-row>
          ...
          <div class="phase-growth">
            ...
            <strong :class="phase2AvgDividend > phase1AvgDividend ? 'green' : 'red'">
              {{
                phase1AvgDividend > 0
                  ? (((phase2AvgDividend - phase1AvgDividend) / phase1AvgDividend) * 100).toFixed(0)
                  : 0
              }}%
            </strong>
          </div>
        ...
        <a-card title="� 分红规则说明" class="rule-card" style="margin-top: 16px">
          <a-alert
            message="两期投资结构：一期本金30w(分红15%) → 二期新增20w(共50w, 分红25%)"
            type="info"
            show-icon
          ...
          <a-descriptions :column="1" size="small" bordered>
            ...
            <a-descriptions-item label="罪老婆"> 一期: 1/6 (≈16.7%) | 二期: 30% </a-descriptions-item>
            ...
            <a-descriptions-item label="剩余分红"> 一期: 2/3 (≈66.7%) | 二期: 40% </a-descriptions-item>
      ...
      <a-row :gutter="[24, 16]">
        ...
        <a-col :xs="24" :md="8">
          <div class="forecast-item">
            ...
            <a-alert :message="riskMessage" :type="riskAlertType" show-icon style="margin-top: 12px" />
          </div>
        </a-col>
      </a-row>
    </a-card>
...
const filteredData = computed(() => {
  ...
  return rawData.value.filter(
    (item) => item.date.includes(searchText.value) || item.totalProfit.toString().includes(searchText.value)
  );
});
...
const phase1AvgDividend = computed(() =>
  phase1Data.value.length > 0 ? phase1TotalDividend.value / phase1Data.value.length : 0
);
const phase2AvgDividend = computed(() =>
  phase2Data.value.length > 0 ? phase2TotalDividend.value / phase2Data.value.length : 0
);
...
const paybackPeriod = computed(() => {
  const months =
    monthlyAvgDividend.value > 0
      ? Math.ceil((totalInvestment - totalDividend.value) / monthlyAvgDividend.value)
      : Infinity;
...
const estimatedPaybackDate = computed(() => {
  ...
  const months =
    monthlyAvgDividend.value > 0 ? Math.ceil((totalInvestment - totalDividend.value) / monthlyAvgDividend.value) : 999;
  const now = new Date();
...
const avgProfitMargin = computed(() => {
  ...
  return recentValid.length > 0
    ? (recentValid.reduce((s, d) => s + d.profitMargin, 0) / recentValid.length).toFixed(1)
    : 0;
});
...
function columnSummary(column) {
  ...
  const sum = rawData.value.reduce((s, r) => s + (r[key] || 0), 0);
...
function clearData() {
  ...
  if (confirm('确定要清除所有数据吗？此操作不可恢复！')) {
    try {
      ...
      rawData.value.forEach((item) => {
        item.profitMargin =
          item.totalProfit > 0 ? Number(((item.totalDividend / item.totalProfit) * 100).toFixed(1)) : 0;
      });
...
.payback-progress .progress-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: 12px;
...
.rule-card :deep(.ant-descriptions-item-label) {
  width: 130px;
}
...
.phase-growth strong.green {
  color: #52c41a;
  font-size: 18px;
  margin-left: 8px;
}
.phase-growth strong.red {
  color: #ff4d4f;
  font-size: 18px;
  margin-left: 8px;
}
